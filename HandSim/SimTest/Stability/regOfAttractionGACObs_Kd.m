function r = regOfAttractionGACObs(q,qDot,qTilde,Kd)
    % Set model variables:
    kP = 0.043;
    kD = 0.185;
    dP = 0.01;
...
%     Kd = 500;
    ...
    lambdaD = eig(D);
    % Get circle radius
    r = (1/(2*kc))*sqrt(min(lambdaL)/max(lambdaL))*(Kd*min(lambdaM)+min(lambdaD)-(((max(lambdaHd)^2)/4)*((1/(min(lambdaD)+min(lambdaHd)-min(lambdaM)-(kc*qTilde)))+(1/(min(lambdaHp)-min(lambdaKi))))));
end
